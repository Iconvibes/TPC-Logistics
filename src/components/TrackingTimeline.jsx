  ...
  if (!events || events.length === 0) {
    return (
      <div className="rounded-3xl border border-slategray-700/40 bg-navy-800/60 p-6 text-sm text-slategray-100">
        No tracking updates have been posted yet.
      </div>
    )
  }
        ...
        {events.map((event) => (
          <div key={event.id} className="relative pl-12">
            <div className="absolute left-0 top-4 flex h-8 w-8 items-center justify-center rounded-full border border-slategray-700/60 bg-navy-900">
              <span className={`h-3 w-3 rounded-full ${dotStyles[event.status] || dotStyles.Created}`} />
            </div>
                ...
                <div className="text-right">
                  <span
                    className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${
                      statusStyles[event.status] || statusStyles.Created
                    }`}
                  >
                    {event.status}
                  </span>
